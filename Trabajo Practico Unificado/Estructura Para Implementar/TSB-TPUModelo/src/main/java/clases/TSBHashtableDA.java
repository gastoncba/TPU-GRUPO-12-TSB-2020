...
public class TSBHashtableDA<K,V> implements Map<K,V>, Cloneable, Serializable
{
    ...
    public V get(Object key) 
    {
        ...
        Entry x = (Entry) this.search_for_entry((K)key, ik);

        if(x != null && x.getState() != TOMBSTONE) {
    ...
    public V put(K key, V value)
    {
       ...
       if(x != null) 
       {
           ...
           //verificamos si el entry que se obtuvo estaba en estado tombstone
           if (x.getState() == TOMBSTONE) {
               //si estaba tumba le cambiamos el valor a cerrado...
               x.setState(CLOSED);
           }
       }
       else
    ...
    public V remove(Object key) 
    {
        ...
        //obtenemos el indice de la tabla en donde se aloja el value con la clave dada
        int ib =  this.h(key.hashCode());

        // ahora se debe buscar en la tabla un objeto Entry cuya clave coincida con key, a partir
        //de la posición ib. Si lo encuentra, retorna su posicíón. Si no lo encuentra,
        //retorna -1. Aplica exploración cuadrática.

        int ik = search_for_index((K)key,ib);

        //si ik es distinto de -1 entonces entonces hay un objeto Entry que coincide con esa clave

        V oldValue = null;

        if (ik != -1) {

            Entry oldEntry = (Entry) table[ik];
            oldEntry.setState(TOMBSTONE);
            oldValue = (V) oldEntry.getValue();

            //se disminuye la cantidad de elementos
            this.count --;

            //se aumentan los cambios
            this.modCount++;
        }
    ...
    public void clear() 
    {
        ...
        //s inicializa el arreglo de soporte con la capacidad inicial
        this.table = new Object[initial_capacity];

        //se llena el vector de soporte con entry vacias
        for (int i = 0; i < table.length ; i++) {

            table[i] = new Entry<K,V>(null, null);
        }

        //se resetea el contador de elementos en cero y se aumenta modCount
        this.count = 0;
        this.modCount++;
    ...
    protected Object clone() throws CloneNotSupportedException 
    {
        ...
        //hace una nueva tabla en la hashtable clonada, que tenga el tamaño que el arreglo de soporte de la hashtable original
    ...
    private class KeySet extends AbstractSet<K> 
    {
        ...
        private class KeySetIterator implements Iterator<K>
        {
            ...
            public KeySetIterator()
            {
                ...
                actual = -1;
                cantElementosTotales = TSBHashtableDA.this.count;
                cantElementosRecorridos = 0;
            }
            ...
            {
                //si la hashtable esta vacia retornar falso...
            ...
            public K next() 
            {
                ...
                while (entry.getState() == OPEN || entry.getState() == TOMBSTONE) {

                    actual++;
                    entry = (Entry<K, V>) TSBHashtableDA.this.table[actual];

                }

                //si sale del ciclo o entrar significa que el entry esta abierta y por lo tanto hay algo que mostrar...
                //aumentamos la cantidad de elementos abietos recorridos
                cantElementosRecorridos++;
    ...
    private class EntrySet extends AbstractSet<Map.Entry<K, V>> 
    {
        ...
        public boolean contains(Object o) 
        {
            ...
            //llegado hasta aqui el parametro o no es null y efectivamente es de la clase Entry
            //lo cual se procede a buscarlo

            Map.Entry e, k;
            e = (Map.Entry) o;
            int i = TSBHashtableDA.this.h((K) e.getKey());

            k = TSBHashtableDA.this.search_for_entry((K) e.getKey(),i);

            //si k es distinto de null, quiere decir que existe un entry en esa posicion

            if (k!=null) {

                if (k.getValue().equals(e.getValue())) return true;
        ...
        public boolean remove(Object o) 
        {
            ...
            if (k!=null) {
                TSBHashtableDA.this.remove(k.getKey());
                TSBHashtableDA.this.count--;
                TSBHashtableDA.this.modCount++;
                return true;
            }
        ...
        private class EntrySetIterator implements Iterator<Map.Entry<K, V>>
        {
            ...
            public boolean hasNext() 
            {
                //si la hashtable esta vacia retornar false
                if (TSBHashtableDA.this.isEmpty()) {return false;}
